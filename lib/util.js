...
class Util {
    ...
    /**
     * Reports error to the sender of the specified message and rethrows it.
     * @param {Message} message
     * @param {Error} error
     * @returns {Promise}
     */
    static reportAndRethrow(message, error) {
        return Promise.resolve()
            .then(() => Util.replyWithErrorEmbed(message, error.message))
            .then(() => {
                throw error;
            });
    }
